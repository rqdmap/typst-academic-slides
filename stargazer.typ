...
#let title-slide(config: (:), ..args) = touying-slide-wrapper(self => {
  ...
  let body = {
    ...
    grid(
      ...
      column-gutter: 1em,
      row-gutter: 1em,
      ..info.authors.map(author => text(fill: black, size: .85em, author)),
    )
    v(0.5em)
    // institution
    if info.institution != none {
      parbreak()
      text(size: .85em, info.institution)
    }
    // date
    if info.date != none {
      parbreak()
      text(size: .85em, utils.display-info-date(self))
    }
  }
...
#let outline-slide(
  ...
  self = utils.merge-dicts(
    self,
    config,
    config-common(freeze-slide-counter: true), // 添加这一行
  )
  self.store.title = title
  touying-slide(
    self: self,
...
///
/// ```typst
/// #show: stargazer-theme.with(aspect-ratio: "16-9", config-colors(primary: blue), logo-path: "path/to/logo.jpg", logo-height: 3.5em)
/// ```
///
/// Consider using:
///
...
///
/// - logo-path (string): is the path to the logo image. The default is `none`.
///
/// - logo-height (length): is the height of the logo. The default is `3.5em`.
///
      ...
      block(
        ...
        height: auto,
        {
          ...
          if self.store.logo-path != none {
            // 根据是否有标题设置不同的位置策略
            if self.store.title != none {
              // 有标题的页面（常规页面）：logo垂直居中
              place(
                right + horizon,
                image(self.store.logo-path, height: self.store.logo-height),
                dx: 0em,
                dy: -0.65em,
              )
            } else {
              // 无标题的页面（首页）：logo水平居中，垂直位置向下调整
              place(
                right + top,
                image(self.store.logo-path, height: self.store.logo-height),
                dx: 0em,
                dy: -1.75em,  // 向下偏移使其在首页看起来更居中
              )
            }
          }
        }
